...
it('serializeTx() and desrializeTx() should be deterministic', function () {
  ...
  var txobj = bitcoinlikejs.transaction.createRawTx(
    [{
  ...
  var txobj_serialized = bitcoinlikejs.transaction.serializeTx(txobj)
  var txobj_deserialized = bitcoinlikejs.transaction.deserializeTx(txobj_serialized)
...
it('signTx() should be deterministic', function () {
  ...
  const SIGHASH_ALL = 1
  var signedobj = bitcoinlikejs.transaction.signTx(txobj, 0, '2c3a48576fe6e8a466e78cd2957c9dc62128135540bbea0685d7c4a23ea35a6c', compressPubKey, SIGHASH_ALL)
  var signed_serialized = bitcoinlikejs.transaction.serializeTx(signedobj)
...
it('multiSign() for x-of-5 should be deterministic', function () {
  ...
  var privKeys = privKeysWIF.map((x) => bitcoinlikejs.address.WIFToPrivKey(x))
  var pubKeys = privKeys.map((x) => bitcoinlikejs.address.privKeyToPubKey(x, true))
  // var addresses = pubKeys.map((x) => bitcoinlikejs.address.pubKeyToAddr(x, '2098'))
  var redeemScript = bitcoinlikejs.address.mkMultiSigRedeemScript(pubKeys, 2, 5)
  // var multiSigAddress = bitcoinlikejs.address.multiSigRSToAddress(redeemScript, '2092')
  ...
  var sig1 = bitcoinlikejs.transaction.multiSign(txobj, 0, privKeys[0], redeemScript)
  var sig2 = bitcoinlikejs.transaction.multiSign(txobj, 0, privKeys[1], redeemScript)
  var tx0 = bitcoinlikejs.transaction.applyMultiSignatures(txobj, 0, [sig1, sig2], redeemScript)

  // Serialize the transaction
  var serializedTx = bitcoinlikejs.transaction.serializeTx(tx0)
...
it('multiSign() and applyMultiSignatures() and should be deterministic', function () {
  ...
  var priv1 = bitcoinlikejs.address.WIFToPrivKey(privKeysWIF[0])
  var priv2 = bitcoinlikejs.address.WIFToPrivKey(privKeysWIF[1])
  var priv3 = bitcoinlikejs.address.WIFToPrivKey(privKeysWIF[2])
  ...
  var sig1 = bitcoinlikejs.transaction.multiSign(txobj, 0, priv3, redeemScript)
  var sig2 = bitcoinlikejs.transaction.multiSign(txobj, 0, priv2, redeemScript)
  ...
  expect(bitcoinlikejs.transaction.deserializeTx(serializedTx)).to.deep.equal(deserializedTx)
})
...
it('addressToScript should be deterministic for both P2SH and P2PKH on mainnet and testnet', function () {
  ...
  const blockHeight = 142091
  const blockHash = '00000001cf4e27ce1dd8028408ed0a48edd445ba388170c9468ba0d42fff3052'
  expect(bitcoinlikejs.transaction.addressToScript(addr1, blockHeight, blockHash, '')).to.equal('a914ed3f0f01c90f41ff665570d38f070c1ee8c075fe87205230ff2fd4a08b46c9708138ba45d4ed480aed088402d81dce274ecf01000000030b2b02b4');
  expect(bitcoinlikejs.transaction.addressToScript(addr2, blockHeight, blockHash, '')).to.equal('76a9143bc25502467ba509b9fb3680148a12b89c56247088ac205230ff2fd4a08b46c9708138ba45d4ed480aed088402d81dce274ecf01000000030b2b02b4');
  expect(bitcoinlikejs.transaction.addressToScript(addr3, blockHeight, blockHash, '')).to.equal('76a914ab523674d9f2ed5a0b300aeb072fc09801363f9f88ac205230ff2fd4a08b46c9708138ba45d4ed480aed088402d81dce274ecf01000000030b2b02b4');
})
